...
Go
--Earnings by City
Create view vw_EarningsPerYearByCity
As
Select Year(BusScheduleAssignmentDate) [Year], BusStopCity, Sum(fareAmount * Riders) [total Earnings]
from BusStop bs
inner join BusRouteStops brs
on bs.BusStopKey=brs.BusStopKey
inner join BusScheduleAssignment bsa
on brs.BusRouteKey=bsa.BusRouteKey
inner join Ridership r
on r.BusScheduleAssigmentKey=bsa.BusScheduleAssignmentKey
inner join Fare f
on f.FareKey=r.FareKey
Group by  BusStopCity,Year(BusScheduleAssignmentDate)
...
As
Select year(BusScheduleAssignmentDate) [Year], Sum(fareAmount * riders) Total
From BusScheduleAssignment bsa
inner join ridership r
on r.BusScheduleAssigmentKey=bsa.BusScheduleAssignmentKey
inner join Fare f
